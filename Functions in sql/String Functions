...
SELECT product_nae,price AS sec_high_price
FROM products 
WHERE price = ( 
SELECT MAX(price) 
FROM products 
...
--STRING FUNCTION
--1. Get all categories in upper case
SELECT UPPER(category) AS categ_Capital
FROM products;

--2.  Get all categories in Lower case
SELECT LOWER(category) AS categ_Capital
FROM products;

--3. Join product name and category using hyphen
SELECT CONCAT(product_name,'-',category) AS product_details
FROM products;

--4. extract first five character from product name
SELECT SUBSTRING(product_name,1,5) AS first_fve
FROM products;

--another way
SELECT LEFT(product_name,5) AS first_fve
FROM products;
...
--6. removing leading nad trailing spaces
SELECT TRIM('   hello   ') AS trimed_text;
SELECT LENGTH('   hello   ') AS trimed_text;
SELECT LENGTH(TRIM('   hello   ')) AS trimed_text;

--7. replace the word phone with device from product name
SELECT REPLACE(product_name,'phone','device') 
AS update_column
FROM products;

--8. find position of  mouse in product name column
SELECT product_name,
       POSITION('Mouse' IN (product_name)) AS position_
FROM products;
